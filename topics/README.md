...
1. **Lesson — this is where the real code lives.** The concepts (every one mapped back to what you know from Node/TS) *and the build itself*: each new file of the app is introduced here, explained line by line. Don't just read it — **type the code in as you go**; by the end of a lesson the app has visibly grown.
2. **Exercises & Solutions — validation, not new theory.** Drills that prove, break, and stress what the lesson just built: produce the race condition, read the exact compiler error, watch the 401 become a 403. Occasionally an exercise adds one small feature (a deposit endpoint) — but the topic's core machinery is always built in the lesson. Each task is followed by its full solution plus the interview talking point it was secretly teaching. Attempt each exercise before reading its solution; the differences only stick when the compiler yells at you.

Topics 1–4 use small console programs. Topics 5–10 are one continuous build of a **Payment API** (register, login, transfer, balance — users and accounts in Postgres): **Topic 5** births it straight onto Postgres (via docker-compose) — written once, no throwaway in-memory phase → **Topic 6** unpacks what Topic 5 wired cookbook-style (DbContext, change tracking, migrations) and adds tests → **Topic 7** races the transfer endpoint, loses money, and fixes it → **Topic 8** containerizes and ships it (api + db in compose) → **Topic 9** adds register/login/JWT and locks the money endpoints to their owners → **Topic 10** rebuilds the plumbing production-style: middleware, declarative validation, and an external Node **payment processor** (provided ready-made) that the API calls over HTTP — concurrent two-leg transfers, per-account locks, a deadlock produced on purpose, and a background auditor. Each topic starts exactly where the previous one left off — don't skip ahead.
