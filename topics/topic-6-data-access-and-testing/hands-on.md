...
1. Restart harder than 5.1 did: `docker compose down`, then `up -d`, and re-read the users' balances. Did the data survive killing the *database container*? Why — and which compose line is responsible?
2. Inspect the table from inside the container:
   ...
   Where did `numeric` and `text` come from? Why is `"Users"` quoted? Find the unique index from Concepts — and identify the one table you never created.
...
2. `\d "Users"` shows something like:
...
2. Add a test that transferring to an **unknown payee** throws `KeyNotFoundException` and moves no money.
...
```csharp
[Fact]
public async Task TransferAsync_Throws_ForUnknownPayee_AndMovesNothing()
{
    var db = NewDb();
    var alice = await NewAuth(db).RegisterAsync(new RegisterRequest("Alice", "a@t.t", "x"));

    await Assert.ThrowsAsync<KeyNotFoundException>(
        () => NewPayment(db).TransferAsync(alice.Id, 999, 100m));

    Assert.Equal(1000m, await BalanceOf(db, alice.Id));   // untouched — payer read, payee missing, nothing saved
}
...
public async Task TransferAsync_ConservesMoney(decimal amount)
{
    ...
    await NewPayment(db).TransferAsync(alice.Id, bob.Id, amount);

    var total = await BalanceOf(db, alice.Id) + await BalanceOf(db, bob.Id);
